...
$echo theta_w_h(r,h)=0; theta_w_h(r,h)$w0_h(r,h) = lsr0(r,h)/w0_h(r,h);	>>MCPMODEL.GEN

$macro CW(r,h)	( (theta_w_h(r,h)*PLS(r,h)**(1-esub_zh(r,h)) + (1-theta_w_h(r,h))*PZ(r,h)**(1-esub_zh(r,h)))**(1/(1-esub_zh(r,h))) )
$macro DZ(r,h)	( (CW(r,h)/PZ(r,h))**esub_zh(r,h) )
$macro DLSR(r,h)	( (CW(r,h)/PLS(r,h))**esub_zh(r,h) )
...
mkt_PC(r,h).. 	C(r,h)*c0_h(r,h) =g= Z(r,h)*c0_h(r,h);

mkt_PINV(r).. 	INV(r)*inv0(r) =g= sum(h,Z(r,h)*inv0_h(r,h));
