...
parameter theta_z(r);
$echo theta_z(r)=0; theta_z(r)$w0(r) = leis_e0(r)/z0(r);	>>MCPMODEL.GEN

$macro CZ(r)	( (theta_z(r)*PLS(r)**(1-esub_z(r)) + (1-theta_z(r))*PC(r)**(1-esub_z(r)))**(1/(1-esub_z(r))) )
$macro DC(r)	( (CZ(r)/PC(r))**esub_z(r) )
$macro DLSR(r)	( (CZ(r)/PLS(r))**esub_z(r) )

prf_Z(r)..		CZ(r) - PZ(r) =g= 0;
...
$macro CW(r)	( (theta_w(r)*PINV(r)**(1-esub_w(r)) + (1-theta_w(r))*PZ(r)**(1-esub_w(r)))**(1/(1-esub_w(r))) )
$macro DZ(r)	( (CW(r)/PZ(r))**esub_w(r) )
$macro DSAV(r)	( (CW(r)/PINV(r))**esub_w(r) )

prf_W(r)..		CW(r) - PW(r) =g= 0;
* prf_W(r)..		PZ(r)*z0(r) + PINV(r)*inv0(r) =g= PW(r)*w0(r);
...
*				+ sum(s, RK(r,s)*ksrs_m(r,s) + RKX(r,s)*ks_x(r,s))
				...
				+ sum(s$kd0(r,s),VA(r,s)*tk(r,s)*RK(r,s)*KD(r,s))
				+ sum(s$kd0(r,s),YX(r,s)*tk(r,s)*RKX(r,s)*kd0(r,s))
				+ PCO2*carb0(r);
...
mkt_PC(r).. 	C(r)*c0(r) =g= Z(r)*c0(r)*DC(r);

mkt_PINV(r).. 	INV(r)*inv0(r) =g= W(r)*inv0(r)*DSAV(r);
